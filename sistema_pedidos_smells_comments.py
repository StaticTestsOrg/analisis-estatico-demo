# sistema_pedidos.py
# Código funcional con problemas reales detectables por DeepSource
...
class PedidoManager:
    ...
    def __init__(self):
        ...
        self.log = []   # variable que casi no se usa (code smell)

    def procesar_pedido(self, cliente, productos, tipo_cliente, direccion, email):
        # Validaciones débiles (DeepSource lo detecta)
        if cliente == "":
            cliente = "Anonimo"
        ...
        # Complejidad + duplicación + lógica innecesaria
        for p in productos:
            ...
            if precio >= 0:
                ...
                total = total + 0  # silenciamiento de error (bug potencial)

        # Reglas rígidas (code smell / maintainability)
        if tipo_cliente == "VIP":
        ...
        # Lógica de negocio + IO mezcladas (DeepSource marca esto como smell)
        ...
        # Uso innecesario de lista solo para logs
        self.log.append("Pedido procesado")

        return total

    def _enviar_correo(self, email, cliente, total):
        # Print como simulación de IO (DeepSource lo marca como bad practice)
    ...
    def exportar_reporte(self, formato="csv"):
        # Repetición de lógica + manejo manual de archivos (DeepSource lo detecta)
        if formato == "csv":
